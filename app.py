...
def fetch_data_safe(stock_dict):
    ...
    for ticker, ch_name in stock_dict.items():
        try:
            ...
            # 取得最新交易日期
            trade_date_str = df.index[-1].strftime("%Y/%m/%d")
...
with st.spinner("資料載入中，請稍候..."):
    df_res, data_date = fetch_data_safe(STOCK_DICT)
...
if df_res.empty:
    ...
    if sort_by == "市值大小 (Market Cap)":
        df_sorted = df_res.sort_values(
            by=["市值(億)", "綜合評分"], ascending=[False, False]
        )
    else:
        df_sorted = df_res.sort_values(
            by=["綜合評分", "市值(億)"], ascending=[False, False]
        )
    ...
    fig.update_layout(
        ...
        xaxis_rangeslider_visible=False,  # 隱藏預設 Slider 保持畫面簡潔
        showlegend=True,
    )
